...
import pandas as pd
from api.db import Session1, engine1
...
from models.people import DBEmployee, Info, Login
from models.places import DBPlace, Place
from packages.data.methods import (
    create_place,
    get_db,
    get_employee_a,
    get_place,
...
@v0.post("/info/")
def info_view(
    info: Info, credentials: HTTPAuthorizationCredentials = Security(security)
):
    token = credentials
    data = {}
...
#     srn: int, credentials: HTTPAuthorizationCredentials = Security(security), db: Session = Depends(get_db2)   # noqa: E501
# ):
#     # return id
#     print(credentials)
...
@v0.post("/login/")
async def get_login_view(token: str):
    import jwt
